-- Add transactional email events for real shipped/delivered order states.
-- This migration extends the existing outbox flow without changing its
-- idempotency model or resending historical emails.
...
begin
  ...
  return jsonb_strip_nulls(jsonb_build_object(
    ...
    'deliveredAt', order_record.delivered_at,
    'items', items_value
  ));
end;
$$;
...
create or replace function public.enqueue_order_fulfillment_email()
returns trigger
language plpgsql
security definer
set search_path = public
as $$
begin
  if old.status is distinct from 'shipped'
    and new.status = 'shipped'
    and new.shipped_at is not null then
    perform public.enqueue_order_email(new.id, 'order_shipped');
  end if;

  if old.status is distinct from 'delivered'
    and new.status = 'delivered'
    and new.delivered_at is not null then
    perform public.enqueue_order_email(new.id, 'order_delivered');
  end if;
...
drop trigger if exists enqueue_order_fulfillment_email on public.orders;
create trigger enqueue_order_fulfillment_email
after update of status, shipped_at, delivered_at on public.orders
for each row
execute function public.enqueue_order_fulfillment_email();
...
revoke execute on function public.enqueue_order_fulfillment_email() from public;
revoke execute on function public.enqueue_order_fulfillment_email() from anon;
revoke execute on function public.enqueue_order_fulfillment_email() from authenticated;
